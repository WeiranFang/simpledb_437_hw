...
public class BufferPool {
    ...
    public BufferPool(int numPages) {
        //IMPLEMENT THIS
        frames = new Page[numPages];
        pinCount = new int[numPages];
        //dirty = new boolean[numPages];
        //pinCount = new HashMap<Page, Integer>();
    ...
    public synchronized Page getPage(TransactionId tid, PageId pid, Permissions perm)
        ...
        if (!permission.equals("READ_ONLY") && !permission.equals("READ_WRITE")) {
            throw new IOException();
        }
        // search existing pages
        int target = 0;
        while (target < frames.length && frames[target] != null) {
            Page page = frames[target];
            if (page.id().equals(pid)) {
                // existing page is found
                lru.remove((Integer) target);
                lru.addLast(target);
                //pinCount[target]++;
                //dirty[target] = false;
                _numhits++;
        ...
        _nummisses++;
        // if no free frame in buffer pool
        if (target == frames.length) {
            target = evictPage();
        }

        if (target == -1) {
            throw new DbException("Unable to find free page.");
        }

        // read page from disk
        try {
            int tableid = pid.tableid();
            Catalog catalog = Database.getCatalog();
            DbFile dbFile = catalog.getDbFile(tableid);
            Page retrieved = dbFile.readPage(pid);
            frames[target] = retrieved;
            lru.remove((Integer) target);
            lru.addLast(target);
            //pinCount[target]++;
            //dirty[target] = false;
